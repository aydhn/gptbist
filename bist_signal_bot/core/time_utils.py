...
ISTANBUL_TZ = ZoneInfo(get_settings().DEFAULT_TIMEZONE)
...
def to_naive_istanbul(dt: datetime) -> datetime:
    ...
    istanbul_dt = ensure_timezone(dt, get_settings().DEFAULT_TIMEZONE)
...
def combine_date_time_istanbul(day: date, hhmm: str) -> datetime:
    ...
    return ensure_timezone(dt, get_settings().MARKET_TIMEZONE)
