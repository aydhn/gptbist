    ...
    def default_policy(self, settings: Settings | None = None) -> GovernancePolicy:
        ...
        rules = [
            GovernanceRule(
                ...
                blocking=bool(getattr(cfg, "GOVERNANCE_WARN_ON_MISSING_DISCLAIMER", True)),
        ...
        return GovernancePolicy(
            ...
            rules=rules,
            require_research_only=bool(getattr(cfg, "GOVERNANCE_REQUIRE_RESEARCH_ONLY", True)),
            block_real_order_language=bool(getattr(cfg, "GOVERNANCE_BLOCK_REAL_ORDER_LANGUAGE", True)),
            block_broker_api=bool(getattr(cfg, "GOVERNANCE_BLOCK_BROKER_API", True)),
            block_paid_services=bool(getattr(cfg, "GOVERNANCE_BLOCK_PAID_SERVICES", True)),
            block_html_scraping=bool(getattr(cfg, "GOVERNANCE_BLOCK_HTML_SCRAPING", True)),
            require_secret_redaction=bool(getattr(cfg, "GOVERNANCE_REQUIRE_SECRET_REDACTION", True)),
            require_confirm_for_policy_update=bool(getattr(cfg, "GOVERNANCE_REQUIRE_CONFIRM_FOR_POLICY_UPDATE", True)),
        )
