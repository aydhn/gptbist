...
class RegimeClassifier:
    ...
    def _classify_momentum(self, score: float) -> MomentumRegime:
        if score >= 85:
            return MomentumRegime.EXTREME_POSITIVE
        if score >= 60:
    ...
    def _determine_market_regime(
        ...
        if trend in (TrendRegime.STRONG_UPTREND, TrendRegime.UPTREND):
            if vol in (VolatilityRegime.LOW, VolatilityRegime.NORMAL) and liq in (
                LiquidityRegime.STRONG,
    ...
    def build_reasons(
        self, feature_set: RegimeFeatureSet, market_regime: MarketRegime
    ) -> list[str]:
        reasons = []
    ...
    def classify(
        self, feature_set: RegimeFeatureSet, config: RegimeConfig | None = None
    ) -> RegimeClassification:
        ...
        market_regime = self._determine_market_regime(
            trend, vol, liq, mom, feature_set.breakout_score
        )
    ...
    def classify_dataframe(
        self, data: "pd.DataFrame", symbol: str, config: RegimeConfig | None = None
    ) -> RegimeClassification:
