...
class DisclosureRecord(BaseModel):
    ...
    language: str = "tr"
    status: DisclosureProcessingStatus = DisclosureProcessingStatus.RAW_IMPORTED
    ...
    def validate_record(self) -> "DisclosureRecord":
        if not self.title.strip() or not self.body.strip():
            raise ValueError("title and body cannot be entirely empty")
        ...
        if not self.source:
            if "source_missing" not in self.warnings:
                self.warnings.append("source_missing")
...
class DisclosureImpactAssessment(BaseModel):
    ...
    recommended_decision: str = "NO_ACTION"
