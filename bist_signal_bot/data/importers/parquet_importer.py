...
class ParquetMarketDataImporter:
    ...
    def read_parquet(self, path: Path) -> pd.DataFrame:
        try:
            ...
            raise DataImportError(f"Failed to read Parquet file {path}: {e}")
    ...
    def import_file(self, request: ImportRequest) -> ImportResult:
        ...
        symbol = request.symbol or path.stem.upper()
        if not symbol:
             return ImportResult(
                request=request,
                status=DataImportStatus.FAILED,
        ...
        try:
            ...
            return ImportResult(
                request=request,
                status=DataImportStatus.SKIPPED,
                symbol=symbol,
                timeframe=request.timeframe,
                warnings=["pyarrow is required to read parquet files. Import skipped."]
            )
        ...
        try:
            ...
            lineage = DataLineageSource(
                ...
                provider_name="Parquet Importer",
                symbol=symbol,
                timeframe=request.timeframe,
