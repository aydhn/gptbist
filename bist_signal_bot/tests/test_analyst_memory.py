...
def test_memory_builder(tmp_path):
    ...
    similarity = KnowledgeSimilarityEngine(search_engine, settings)
    lib = ResearchCaseLibrary(similarity, settings)
    builder = AnalystMemoryBuilder(lib, store, settings)

    card = builder.build_memory_card("ASELS", "momentum")
