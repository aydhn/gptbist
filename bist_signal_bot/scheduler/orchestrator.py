...
class LocalSchedulerOrchestrator:
    ...
    def run_due(
        self, now: datetime | None = None, dry_run: bool = False, limit: int | None = None
    ) -> SchedulerRunResult:
        if now is None:
            now = datetime.utcnow()
        ...
        result = SchedulerRunResult(
            scheduler_run_id=run_id, started_at=started_at, status=ScheduledJobStatus.RUNNING
        )
        ...
        if not self.lock_manager.acquire_lock(global_lock_name, global_lock_ttl):
            self.logger.warning(
                "Could not acquire global scheduler lock. Another run_due is likely active."
            )
        ...
        try:
            due_result, jobs_to_run = self._find_and_filter_due_jobs(now, result, limit)
            result.due_result = due_result

            gate_blocked = self._check_config_gate(result)
            if gate_blocked:
                jobs_to_run = []

            self._execute_jobs(jobs_to_run, now, dry_run, result)
    ...
    def _find_and_filter_due_jobs(
        self, now: datetime, result: SchedulerRunResult, limit: int | None
    ) -> tuple[Any, list[ScheduledJob]]:
        ...
        unique_due, dupes = self.deduplicator.filter_duplicate_due_jobs(
            due_jobs, recent_runs, dedupe_window
        )
        ...
        if len(unique_due) > actual_limit:
            result.warnings.append(
                f"Limited run to {actual_limit} out of {len(unique_due)} due jobs"
            )

        return due_result, jobs_to_run

    def _check_config_gate(self, result: SchedulerRunResult) -> bool:
        try:
    ...
    def _execute_jobs(
        ...
        result.skipped_count = len(result.due_result.skipped_jobs)
        result.blocked_count = len(result.due_result.blocked_jobs)
