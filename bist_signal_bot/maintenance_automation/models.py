...
class MaintenanceAction(BaseModel):
    ...
    def validate_action(self):
        if self.destructive and not self.requires_confirm:
            raise ValueError("Destructive action must require confirm.")
        if self.command:
            unsafe = ["broker", "live", "order", "buy", "sell"]
            if any(u in self.command.lower() for u in unsafe):
                if "BLOCKED: Contains potentially unsafe live/broker command." not in self.warnings:
                    self.warnings.append("BLOCKED: Contains potentially unsafe live/broker command.")
        return self
...
class MaintenanceRunManifest(BaseModel):
    ...
    plan_id: str
    created_at: datetime
    action_result_ids: List[str] = Field(default_factory=list)
    affected_paths: List[str] = Field(default_factory=list)
    deleted_paths: List[str] = Field(default_factory=list)
    archived_paths: List[str] = Field(default_factory=list)
...
class MaintenanceRun(BaseModel):
    ...
    finished_at: Optional[datetime] = None
    status: MaintenanceStatus
    results: List[MaintenanceActionResult] = Field(default_factory=list)
