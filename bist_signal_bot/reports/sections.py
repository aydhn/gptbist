...
from bist_signal_bot.reports.models import (
    ReportDataBundle,
    ReportConfig,
    ReportSection,
    ReportSectionType,
...
class ReportSectionBuilder:
    ...
    def build_executive_summary(
        self, bundle: ReportDataBundle, config: ReportConfig
    ) -> ReportSection:
        return ReportSection(
    ...
    def build_market_regime_section(
        ...
        return ReportSection(
            section_id="market_regime",
            section_type=ReportSectionType.MARKET_REGIME,
            title="Market Regime",
            body_markdown="Regime analysis.",
        )

    def build_scanner_highlights_section(
        self, bundle: ReportDataBundle, config: ReportConfig
    ) -> ReportSection:
        return ReportSection(
            section_id="scanner_highlights",
            section_type=ReportSectionType.SCANNER_HIGHLIGHTS,
            title="Scanner Highlights",
            body_markdown="Recent scanner candidates.",
        )

    def build_risk_summary_section(
        self, bundle: ReportDataBundle, config: ReportConfig
    ) -> ReportSection:
        return ReportSection(
            section_id="risk_summary",
            section_type=ReportSectionType.RISK_SUMMARY,
            title="Risk Summary",
            body_markdown="Risk metrics.",
        )

    def build_ml_summary_section(
        self, bundle: ReportDataBundle, config: ReportConfig
    ) -> ReportSection:
        return ReportSection(
            section_id="ml_summary",
            section_type=ReportSectionType.ML_SUMMARY,
            title="ML Summary",
            body_markdown="ML prediction analysis.",
        )

    def build_adaptive_summary_section(
        self, bundle: ReportDataBundle, config: ReportConfig
    ) -> ReportSection:
        return ReportSection(
            section_id="adaptive_summary",
            section_type=ReportSectionType.ADAPTIVE_SUMMARY,
            title="Adaptive Summary",
            body_markdown="Adaptive updates.",
        )

    def build_portfolio_research_section(
        self, bundle: ReportDataBundle, config: ReportConfig
    ) -> ReportSection:
    ...
    def build_paper_summary_section(
        self, bundle: ReportDataBundle, config: ReportConfig
    ) -> ReportSection:
        return ReportSection(
            section_id="paper_summary",
            section_type=ReportSectionType.PAPER_SUMMARY,
            title="Paper Trading Summary",
            body_markdown="Simulated execution details. No real orders sent.",
        )

    def build_research_ledger_section(
        self, bundle: ReportDataBundle, config: ReportConfig
    ) -> ReportSection:
        return ReportSection(
            section_id="research_ledger",
            section_type=ReportSectionType.RESEARCH_LEDGER,
            title="Research Ledger",
            body_markdown="Ledger entries.",
        )

    def build_signal_journal_section(
        self, bundle: ReportDataBundle, config: ReportConfig
    ) -> ReportSection:
        return ReportSection(
            section_id="signal_journal",
            section_type=ReportSectionType.SIGNAL_JOURNAL,
            title="Signal Journal",
            body_markdown="Journal entries.",
        )
    ...
    def build_attribution_section(
        ...
        return ReportSection(
            ...
            section_type=ReportSectionType.ATTRIBUTION,
            title="Attribution",
            body_markdown="Attribution reports.",
        )
    ...
    def build_operations_section(
        ...
        return ReportSection(
            ...
            title="Operations",
            body_markdown="System monitoring and operations.",
        )
