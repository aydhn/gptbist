...
from bist_signal_bot.app.ops_app import (
    create_ops_observability_engine, create_store_integrity_checker,
    create_staleness_checker, create_ops_incident_manager,
    create_ops_runbook_registry, create_backup_manager,
    create_restore_planner, create_retention_policy_engine,
    create_migration_checker, create_operational_readiness_gate,
    create_ops_store
)
from bist_signal_bot.ops.models import OpsIncidentType, BackupScope, OpsStatus, OpsReport
from bist_signal_bot.ops.reporting import (
    ops_report_to_dict, format_ops_report_markdown, health_snapshot_to_dict,
    store_integrity_to_dict, staleness_finding_to_dict, incident_to_dict,
    runbook_to_dict, backup_manifest_to_dict, restore_plan_to_dict,
    retention_finding_to_dict, migration_check_to_dict, readiness_to_dict,
    format_health_snapshot_text, format_store_integrity_text, format_incident_text,
    format_runbook_text, format_backup_manifest_text, format_readiness_text
)
import datetime
...
def handle_ops_command(args, settings=None):
    ...
    if args.ops_command == "status":
    ...
    elif args.ops_command == "store-check":
    ...
    elif args.ops_command == "staleness":
    ...
    elif args.ops_command == "incident":
        ...
        if args.rb_command == "list":
            ...
            if rb: print(format_runbook_text(rb))
        elif args.rb_command == "for-incident":
            mgr = create_ops_incident_manager(s)
            inc = mgr.get_incident(args.id)
            if inc:
                rb = reg.runbook_for_incident(inc)
                if getattr(args, "json", False): print(json.dumps(runbook_to_dict(rb), indent=2))
                else: print(format_runbook_text(rb))

    elif args.ops_command == "backup":
        mgr = create_backup_manager(s)
        scopes = [BackupScope(sc.upper()) for sc in args.scope] if getattr(args, "scope", None) else [BackupScope.DATA]
        manifest = mgr.create_backup(scopes=scopes, confirm=not getattr(args, "dry_run", False) and getattr(args, 'confirm', False))
        if getattr(args, "json", False):
            print(json.dumps(backup_manifest_to_dict(manifest), indent=2))
        else:
            print(format_backup_manifest_text(manifest))

    elif args.ops_command == "restore":
        planner = create_restore_planner(s)
        plan = planner.plan_restore(Path(args.backup_path), Path(args.target) if getattr(args, "target", None) else None, dry_run=getattr(args, "dry_run", False))
    ...
    elif args.ops_command == "retention":
    ...
    elif args.ops_command == "migration-check":
    ...
    elif args.ops_command == "readiness":
